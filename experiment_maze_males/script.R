...
# there are 8395 points that have their diff_t > 0.041 out of ~ 1.150.000
dt_clean1[diff_t < 0.03]
dt_clean1 <- dt[, diff_t:= c(0, diff(t)), by=key(dt)]
dt_clean1[diff_t > 0.042]

# param.pca <- dt_clean1[, c("w", "h" , "diff_t", "diff_x", "diff_y"), with =FALSE]
# outliers.pca <- prcomp(param.pca, center = TRUE, scale =TRUE)
# print(outliers.pca)
# summary(outliers.pca)
# condition <-  dt_clean1$sd
# qplot(PC1, PC2, data = fortify(outliers.pca), color = condition)
# ggbiplot(traje.pca,choices=c(1,4), var.scale = 1)


dt_clean1[, diff_x:= c(0, diff(x)), by=key(dt_clean1)]
dt_clean1[, diff_y:= c(0, diff(y)), by=key(dt_clean1)]



dt_cleaned_based_on_w <- dt_clean1[w < 0.05]
dt_cleaned_based_on_w[diff_t > 0.042]

a <- unique(dt_clean1$w)
qplot(a, geom='histogram', binwidth=0.005)

diff_distri <- ggplot(dt_clean1, aes(diff_x)) + geom_histogram(binwidth=10)

ggplot(dt_clean1[diff_t > 0.03 & diff_t < 0.042], aes(diff_t)) + geom_boxplot()
ggplot(dt_clean1[], aes(x='', y=diff_t)) + geom_boxplot()


dt_w <- dt[w < 0.05]
dt_w_clean <- dt_w[, diff_t:= c(0, diff(t)), by=key(dt_w)]
